...
for bin in range(len(skinHistR)):								#risk of dividing by 0 in this block
	pRed.append(float(skinHistR[bin]) / (notSkinHistR[bin]))
	pGreen.append(float(skinHistG[bin]) / (notSkinHistG[bin]))
...
for i in range(densityImg.size[0]):    # for every pixel:
    for j in range(densityImg.size[1]):
		if(pixelData2[i][j][0] > .7 and pixelData2[i][j][1] > .7):
			densityPixels[i,j] = (int(pRed[bins - 1] * 40), int(pGreen[bins - 1] * 40), 0)
		elif(pixelData2[i][j][0] > .7):
			densityPixels[i,j] = (int(pRed[bins - 1] * 40), int(pGreen[int(pixelData2[i][j][1] / (.7 / (bins - 1)))] * 40), 0)
		elif(pixelData[i][j][1] > .7):
			densityPixels[i,j] = (int(pRed[int(pixelData2[i][j][0] / (.7 / (bins - 1)))] * 40), int(pGreen[bins - 1] * 40), 0)
		else:
			densityPixels[i,j] = (int(pRed[int(pixelData2[i][j][0] / (.7 / (bins - 1)))] * 40), int(pGreen[int(pixelData2[i][j][1] / (.7 / (bins - 1)))] * 40), 0)
		'''if(pixelData2[i][j][0] > .7 and pixelData2[i][j][1] > .7):
